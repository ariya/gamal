 ...
 *
 * @param {(text: string) => void} handler - callback function to handle the transcribed text
...
const speak = (text, language) => {
    ...
    try {
        ...
        const speaker = spawn('play', ['-q', '-V0', '-'], { stdio: ['pipe', 'pipe', 'inherit'] });
        ...
        const piper = spawn('piper', ['--quiet', '--model', model, '-f', '-'], { stdio: ['pipe', 'pipe', 'inherit'] });
        piper.on('error', (err) => {
 ...
 * @property {string} buffer - The buffer to flush.
 * @property {Array<number>} refs - The array of citation references.
 * @property {(x: string) => void} print - The function to print the output.
 * @property {(x: string | number) => string} cite - The function to cite a reference.
 */
...
/**
 * TODO: Remove or replace or rename this.
 *
 ...
 * @param {CompletionHandler | null | undefined} handler - An optional callback to stream the completion.
...
const chat = async (messages, handler = null, attempt = MAX_RETRY_ATTEMPT) => {
    ...
    try {
        ...
        if (!stream) {
            ...
            /** @type {Array<{message: {content: string}}>} */
 ...
 *
 * @param {Record<string, string>} kv key-value pairs
 */
const construct = (kv) => {
 ...
 * @property {Array<{url: string}>} references
 ...
 * @property {Array<HistoryItem>} history
 * @property {string} inquiry
 * @property {string} [topic]
 * @property {string} [thought]
 * @property {string} [keyphrases]
 ...
 * @property {Record<string, DelegateHandler>} delegates - Impure functions to access the outside world.
 */
 ...
 * @returns {Promise<Context>} Updated pipeline context.
 */
 ...
 * @return {string | undefined} The corresponding ISO 639-1 language code or null if not found.
 */
 ...
 * @return {Promise<{ url: URL; references: Array<Reference> }>} Array of references containing search results.
...
const review = (stages) => {
    ...
    stages.map((stage, index) => {
        const { name, duration, timestamp, ...other } = stage;
 ...
 * and compute its duration instead of individual timestamps.
 *
 * @param {Array<Stage>} stages
 * @returns {Array<Stage>}
 */
    ...
    return stages
        .map((stage, index) => {
            if (isOdd(index)) {
                ...
                const duration = before ? stage.timestamp - before.timestamp : 0;
        ...
        const handle = async (line) => {
            ...
            if (parts.length >= 2) {
                ...
                if (role === 'Story') {
                    ...
                    /**
                     * @param {string} name
                     * @param {{duration?: number}} fields (TODO: Fixme)
                     */
                    const leave = (name, fields) => {
                        stages.push({ name, timestamp: Date.now(), ...fields });
                    };
                    /** @type Context */
                    const context = { inquiry, history, delegates: { enter, leave } };
                    console.log();
                        ...
                        if (matches.length === regexes.length) {
                            ...
                            if (references && Array.isArray(references)) {
                                if (references.length > 0 && references.length >= refs.length) {
                                    refs.forEach((ref, i) => {
                                        const reference = references[ref - 1];
                                        if (reference) {
                                            const { url } = reference;
 ...
 * @return {Display} The updated display object.
 */
 ...
 *
 * @param {Display} display - The display object.
 * @return {Display} The updated display object with an empty buffer and empty reference array.
 */
const flush = (display) => {
    ...
    const answer = async (inquiry) => {
        ...
        if (inquiry === '!reset' || inquiry === '/reset') {
            ...
            history.push({
                ...
                duration,
                stages,
                // TODO: I added these. Make these two fields optional?
                language: language ?? '',
                references: references ?? [],
            });
            console.log();
 ...
 */
const poll = async () => {
    /** @type Record<number, Array<HistoryItem>> */
    let state = {};
     ...
     * @param {Array<Reference>} references - The array of references.
    ...
    const check = async (offset) => {
        ...
        /**
         * TODO: I'm not sure if `id` here will be a string or a number.
        ...
        try {
            ...
            if (!response.ok) {
                ...
                /** @type any */
                const data = await response.json();
                /** @type Array<{ message: { text: string, chat: { id: number } }, update_id: number }> */
                ...
                result.forEach(async (update) => {
                    ...
                    if (text === '/reset') {
                        ...
                        history.push({
                            ...
                            duration,
                            stages,
                            // TODO: I added this. Make this field optional?
                            language: '',
                        });
